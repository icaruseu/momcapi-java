package eu.icarus.momca.momcapi.model.xml.eag;
...
public class EagDescTest {
    ...
    @NotNull
    private static final EagDesc EAG_DESC = new EagDesc(Util.parseToElement(CORRECT_XML));
    ...
    public void testConstructorWithDetails() throws Exception {
        ...
        EagDesc eagDesc = new EagDesc(countryName, subdivisionName, address, contactInformation, logoUrl);

        assertEquals(eagDesc.toXML(), CORRECT_XML);

    }

    @Test
    public void testConstructorWithXml() throws Exception {
        EagDesc eagDesc = new EagDesc(Util.parseToElement(CORRECT_XML));
        assertEquals(eagDesc.toXML(), CORRECT_XML);
    }
